...
const quickOptionsMenu: QuickOptionsMenu[] = [
  ...
  { label: "Agregar a una clase", action: "ADD_TO_CLASS", icon: FileInput },
];
...
export const listContainerVariants: Variants = {
  ...
  show: {
    opacity: 1,
    transition: { delayChildren: 0.03, staggerChildren: 0.04 },
  },
};
...
export const getFileTypeBadge = (format: FormatType | string) => {
...
export default function ResourceTableView({
  ...
  // ── Recurso seleccionado (compartido entre acciones) ──
  ...
  const [isSubmittingReactivate, setIsSubmittingReactivate] = useState<
    string | null
  >(null);
  ...
  const toggleQuickOptionsMenu = (
    ...
    if (isOpenQO === resourceId) {
      ...
      setSelectedResourceName(resourceTitle);
    }
  };

  const handleActionClick = (action: ResourceMenuAction) => {
    setIsOpenQO(null);
    setMenuPosition(null);
    if (action === "ARCHIVE") setIsArchiveModalOpen(true);
    else if (action === "ADD_TO_CLASS") setIsAddResourceModalOpen(true);
  };
  ...
  const handleArchiveResource = async (
    ...
    try {
      const res = await fetch(`/api/resources/${resourceId}`, {
        ...
        body: JSON.stringify({ status: "archived", visibility: "private" }),
        cache: "no-store",
      });
      if (!res.ok) {
  ...
  const handleReactivateResource = async (resourceId: string) => {
    setIsSubmittingReactivate(resourceId);
    try {
      const res = await fetch(`/api/resources/${resourceId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (error instanceof Error) toast.error(error.message);
      else toast.error("Error al reactivar el recurso.");
    } finally {
      setIsSubmittingReactivate(null);
    }
  };
    ...
    try {
      if (resource.asset.storagePath) {
        ...
        await deleteObject(fileRef).catch((err) =>
          console.warn("El archivo ya no existía en Firebase:", err),
        );
      }
      if (resource.asset.thumbnailStoragePath) {
        ...
        await deleteObject(thumbRef).catch((err) =>
          console.warn("La miniatura ya no existía en Firebase:", err),
        );
      }
              ...
              return (
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {(resource.levels ?? []).map((level, index) => (
                        <span
                          ...
                          className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${getLevelBadge(level)}`}
                        >
                          {level}
                        </span>
                      ))}
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-sm font-medium ${getStatusBadge(resource.status)}`}
                    >
                      {toDisplayLabel(resource.status)}
                    </span>
                  </td>

                  {/* Quick menu — archivados muestran botones inline, activos muestran dots */}
                  <td className="px-6 py-4 text-right">
                    {isArchived ? (
                      <div className="flex items-center justify-end gap-2">
                        <button
                          type="button"
                          disabled={isSubmittingReactivate === resource.id}
                          onClick={() => handleReactivateResource(resource.id)}
                          className="cursor-pointer flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-medium text-emerald-700 transition-all hover:bg-emerald-100 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                          <ArchiveRestore size={12} />
                          {isSubmittingReactivate === resource.id
                            ? "..."
                            : "Reactivar"}
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedResourceName(resource.title);
                            setIsDeleteModalOpen(true);
                          }}
                          className="cursor-pointer flex items-center gap-1.5 rounded-lg border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-medium text-red-600 transition-all hover:bg-red-100"
                        >
                          <Trash2 size={12} />
                          Eliminar
                        </button>
                      ...
                      <button
                        ...
                        className="cursor-pointer menu-button rounded-xl p-2 text-slate-400 transition-colors hover:bg-rose-50 hover:text-[#9e2727]"
                      >
      ...
      >
        <div className="p-4">
          <ArchiveResourceForm
            ...
            isSubmitting={isSubmittingArchive}
            onClose={() => setIsArchiveModalOpen(false)}
          />
        </div>
      </CustomModal>
      ...
      >
        <div className="p-4">
          <DeleteResourceForm
            ...
            isSubmitting={isDeletingResource}
            onClose={() => setIsDeleteModalOpen(false)}
          />
        </div>
      </CustomModal>
